...
[~, idxNodes1] = ismember(nodes1, orimesh(:, 1));
for igCoords = 1:numel(idxNodes1)
    if idxNodes1(igCoords) == 0 
        idNodes1 = idxNodes1(igCoords-1);
        idNodes2 = idxNodes1(igCoords+1);

        gcoord1(igCoords, :) = mean([orimesh(idNodes1, 2:3); orimesh(idNodes2, 2:3)]);
    end
end
...
for i=1:nel
    ...
    if p(i)>1
        for j=2:p(i)
            ...
            elecenter(i).coord(j,:)= supelcentre;
        end
    end
end
...
for i=1:nel
    ...
    idTemp = find(local(i).temps(2) == nodes1);
    local(i).gcoord(2,:) = gcoord1(idTemp, :);
end
...
%         [NN,local(iel).N,local(iel).dhdnita]=lobatto(nita,p(iel));
        [NN,local(iel).N,local(iel).dhdnita]=lagrangian2(nita,p(iel));
        dyds= 0;
