...
function [localcoarse,array,deltaerror,rhs65,error,rhs66,errest]= main21_modify(p, supel1, supel2)
...
error = zeros(nel,1);% contribution of each ele in error eq 6.6
...
for iel=1:nel
    ...
    error(iel)= sqrt(real(-(e1t-e2t-e3t+e4t))); %LHS of eq 6.6
...
array= zeros(nel,1);
for iel=1:nel
    if or(deltaerror(iel) > rhs65, error(iel) > rhs66)
