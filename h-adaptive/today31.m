...
%centre
qx=-1; %1N/mm pointing in x negative direction
...
idx_node2 = find(s(:,3)==2);
idx_node3 = find(s(:,4)==3);

for iel=idx_node2:idx_node3
    ffval_temp = 0;
    for int=1:ngl
        nita= point1(int,1);
        wt= weight1(int,1);
        ...
        ffval_temp = ffval_temp + (local(iel).N)'*qx*abs(dyds)*wt;%abs cos' the distance have to be positive
    end
    if ffval == 0
        ffval = ffval_temp;
    else
