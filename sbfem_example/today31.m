...
for iel=idx_node1:idx_node2
    ...
    for int=1:ngl
        ...
        for j=1:length(local(iel).N)
            dxds= dxds+local(iel).dhdnita(j)*(local(iel).gcoord(j,1)-elecenter(iel).coord(j,1)); % si=1
        end
        ffval_temp = ffval_temp + (local(iel).N)'*qy*abs(dxds)*wt;%abs cos' the distance have to be positive
    end
    if ffval == 0
        ffval = ffval_temp;
    else
