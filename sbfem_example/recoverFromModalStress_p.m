function [recover_str] = recoverFromModalStress_p(sigma_star, supel, local, p)
	...
	for iel=1:nel		
		...
		for i=1:1:length(sinew)
        	for j=1:1:length(nitanew)
					...
					N_for_sigma = [diag(ones(1, 3)*shapelq(1)), diag(zeros(1, 3)*shapelq(2)), diag(ones(1, 3)*shapelq(end))];
